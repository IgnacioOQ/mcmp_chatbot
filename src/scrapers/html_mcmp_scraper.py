...
class HTMLMCMPScraper:
    """
    Parallel scraper that strictly uses only HTML requests (no Selenium),
    intended to run alongside the main scraper to ensure all available 
    static content is merged into the datasets.
    """
            ...
            with open("data/important_urls.txt", "r") as f:
                for line in f:
                    ...
                    if line and not line.startswith("#"):
                        urls.append(line)
            log_info(f"Loaded {len(urls)} important URLs from file (HTML Scraper).")
    ...
    def scrape_people(self):
        ...
        for people_url in sources:
            log_info(f"HTML Scraper starting scrape of people from {people_url}")
            try:
                ...
                log_info(f"HTML Scraper found {len(profiles_to_visit)} potential people profiles to scrape.")

                for url in profiles_to_visit:
                     self._scrape_single_person_page(url)

            except Exception as e:
                log_error(f"HTML Scraper error scraping people index {people_url}: {e}")
    ...
    def scrape_research(self):
        ...
        log_info(f"HTML Scraper starting scrape of {self.RESEARCH_URL}")
        try:
            self.research = [] # Reset
            ...
            log_info(f"HTML Scraper structured research into {len(self.research)} categories.")
            return self.research
            
        except Exception as e:
            log_error(f"HTML Scraper error scraping research: {e}")
            return []
    ...
    def scrape_general(self):
        ...
        log_info(f"HTML Scraper starting scrape of {home_url}")
        try:
            ...
            return self.general
             ...
             log_error(f"HTML Scraper error scraping general info: {e}")
             return []

    def scrape_reading_groups(self):
        """Scrapes reading groups from the events page."""
        url = f"{self.BASE_URL}/mcmp/en/events/index.html"
        log_info(f"HTML Scraper starting scrape of reading groups from {url}")
        try:
            soup = self._fetch_page(url)
            ...
            log_info(f"HTML Scraper scraped reading groups into general/events.")
            return self.general
        except Exception as e:
            log_error(f"HTML Scraper error scraping reading groups: {e}")
            return []
