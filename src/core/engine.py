...
class ChatEngine:
    ...
    def _build_tools_description_str(self) -> str:
        ...
        lines = [
            ...
            "  4. If the query looks like a **misspelled name** (a person, speaker, or research area that should exist but didn't match), call `fuzzy_search(query='<name>', database='people'|'events'|'research'|'all')` — it tolerates typos (e.g. 'Sternkenberg' → 'Tom F. Sterkenburg'). Then re-query the precise tool with the corrected name, and confirm with the user ('Did you mean …?').",
            "  5. Only after exhausting all the steps above may you tell the user nothing was found — and if so, suggest related topics they could try.",
            "",
