...
## 2026-05-06 — Fixed `get_events` dropping `talk_title` from tool returns
...
- **Outcome:** Root cause was in `src/mcp/tools.py::get_events`: the dataset has a dedicated `talk_title` field, but the tool only returned `title` (the listing header, e.g. "Talk: Philipp Haueis (Bielefeld)"), `abstract`, and `description`. The talk's actual title only survived as a `"Title: ..."` prefix inside the long `description` string, which the LLM was failing to parse out reliably. Added `talk_title` to the returned dict (with `""` fallback for conferences/workshops that legitimately have no single title) and updated the docstring to instruct the LLM to prefer `talk_title` when non-empty. Verified: 76 of 89 events have populated `talk_title`; live test against the Haueis entry now returns the correct title field.
- **Key decisions:** (1) Empty-string fallback rather than omitting the key — gives the LLM a uniform shape and avoids implicit `KeyError` paths in any future consumer. (2) No prompt-level change — exposing the structured field is the right fix; massaging the system prompt to "remember to extract Title: from description" would be a workaround for a design bug. (3) Kept `description` and `abstract` in the response for backwards compatibility, even though `description` is now partly redundant with `talk_title + abstract`.
- **KB changes:** None — project-specific fix, not a generalizable lesson worth a KB doc.
- **Follow-up:** Worth a regression case in `tests/stress_test_gemini.py` that asks for a specific upcoming talk and verifies the response contains the `talk_title` text — would catch any future tool-shape regression. Not done in this session.
